...
const { notifyDefiComplete, notifyNouveauNiveau } = require('../services/ryxNotifications');
...
async function _completeQuestCore(quest, userId) {
  ...
  // Capture du niveau AVANT sauvegarde (pour détecter une montée)
  const levelBefore = progress.getLevel();
  ...
  notifyDefiComplete(userId, quest.title, quest.xpReward).catch(() => {});
  if (level.index > levelBefore.index) {
    notifyNouveauNiveau(userId, level.name).catch(() => {});
  }
