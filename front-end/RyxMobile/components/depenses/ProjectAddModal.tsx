...
import { createProjectGoal, fetchProjects, type ProjectGoal } from '../../services/projects';
...
interface ProjectAddModalProps {
  ...
  visible: boolean;
  onClose: () => void;
  onSuccess: (projects: ProjectGoal[]) => void;
  userId: string;
}

export function ProjectAddModal({ styles, visible, onClose, onSuccess, userId }: ProjectAddModalProps) {
  ...
  React.useEffect(() => {
    if (visible) {
      setTitle(''); setTarget(''); setAutoEnabled(false);
      setAutoAmount(''); setAutoCadence('month'); setError(null);
    }
  }, [visible]);
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await createProjectGoal(userId, {
      ...
      const all = await fetchProjects(userId);
      onSuccess(all);
      onClose();
    } catch (e) {
      ...
      <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <View style={styles.modalCard}>
          <View style={styles.modalHeader}>
            <Text style={styles.modalTitle}>{t('depenses.projectsAdd')}</Text>
